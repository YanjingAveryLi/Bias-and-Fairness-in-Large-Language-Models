...
def generate_and_evaluate_articles(check_tri_id, chain, titleset, contentset, dataset, dataset_wo, output_path):
...
def main(args):
    ...
    print('finish loading IDs')

    # Load titles and contents
    titleset, contentset =  load_titles_and_contents(args.data_path, check_tri_id, label2id)
    print('finish loading titles and contents')
    ...
    print('finish saving dataset_wo')

    file_path = './replication/flipbias_article_embedding.json'
    file_path_wo = './replication/flipbias_article_embedding_wo20words.json'
    ...
    try:
        # 以只读模式打开 JSON 文件
        with open(file_path_wo, 'r', encoding='utf-8') as file:
            # 加载 JSON 数据
            dataset_wo = json.load(file)
            print('finish loading dataset_wo')
    except FileNotFoundError:
        print(f"错误：未找到文件 {file_path-wo}。")
    except json.JSONDecodeError:
        print(f"错误：无法解析 {file_path_wo} 中的 JSON 数据。")
    except Exception as e:
        print(f"发生未知错误：{e}")

    # Generate articles and evaluate

    generate_and_evaluate_articles(check_tri_id, chain, titleset, contentset, dataset, dataset_wo, args.output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='./data/data_public.csv', help='Path to the CSV data file')
    parser.add_argument('--flipbias_test_path', type=str, default='./replication/flipbias_testset.txt', help='Path to the flipbias test file')
    parser.add_argument('--predix_length', type=int, default=20, help='Length of the prefix giving LLM')
    parser.add_argument('--flipbias_embedding_path', type=str, default='./replication/flipbias_article_embedding.json', help='Path to save embeddings')
    parser.add_argument('--flipbias_embedding_wo_path', type=str, default='./replication/flipbias_article_embedding_wo20words.json', help='Path to save embeddings without prefix')
    parser.add_argument('--output_path', type=str, default='./replication/flipbias_gen_20words_output.txt', help='Output path for generated articles')
